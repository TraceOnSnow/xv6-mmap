...
void handle_page_fault(struct trapframe *tf)
{
  ...
  for (int i = 0; i < p->total_mmaps; i++)
  {
    ...
    if (p->mmaps[i].flags & MAP_GROWSUP && touch_guard)
    {
      // cprintf("IN MAP_GROWSUP\n");
      // The fault occurred in the guard page. Check the margin constraint.
      // cprintf("WITHIN GUARD\n");
      ...
      // cprintf("INSIDE GUARD, within margin\n");
      ...
      int mmap_result = mmap_store_data(p, PGROUNDDOWN(page_fault_addr), size, p->mmaps[i].flags, p->mmaps[i].protection,
                                        p->mmaps[i].f, p->mmaps[i].offset + PGROUNDDOWN(page_fault_addr) - p->mmaps[i].virt_addr);
      if (mmap_result < 0)
      {
        myproc()->killed = 1;
      }
